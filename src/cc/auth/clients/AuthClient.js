...
export class AuthClient {
  constructor({ baseUrl, config }) {
    if (!config || (!baseUrl && !config.keycloakLogin)) {
      throw new Error(
        'config is required, and baseUrl is required when config is basic auth'
      );
    }
   ...
   * @param {boolean} [options.offline] True if seeking tokens for offline use; false otherwise.
   * @returns {string} Auth URL to open in a browser.
   */
  getSsoAuthUrl({ state, offline = false }) {
    return (
