...
export default class ExtensionRenderer extends LensRendererExtension {
  ...
  protected handleProtocolAddClusters = ({ search }) => {
    ...
    let onlyNamespaces;
    if (search.onlyNamespaces) {
      try {
        onlyNamespaces = JSON.parse(search.onlyNamespaces);
      } catch (err) {
        logger.error(
          'renderer/handleProtocolAddClusters',
          `Failed to decode onlyNamespaces parameter, error="${err.message}"`,
          err
        );
        return;
      }
    ...
    dispatchExtEvent({
      ...
      data: {
        cloudUrl: search.cloudUrl,
        keycloakLogin: search.keycloakLogin && search.keycloakLogin === 'true',
        onlyNamespaces,
